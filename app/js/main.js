...
for (var index = 0; index < process.argv.length; index++) {
  ...
  if (arg.startsWith("-startApp") && index + 1 < process.argv.length) {
    ...
    isStartingGame = true;
    games.startGame(process.argv[index + 1], function () {
...
if (!isStartingGame) {
  ...
  var updateFinished = function updateFinished() {
    createMainWindow();
    updateWindow.close();
  ...
  var createUpdateWindow = function createUpdateWindow() {
  ...
  //Handle add window
  var createAddWindow = function createAddWindow() {
    //Create new window
